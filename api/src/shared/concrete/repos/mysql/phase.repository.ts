  ...
  async getQuestionCount(productId: number, phaseId: number): Promise<any> {
    ...
    try {
      ...
      const QuestionCount = await connection
        ...
        .andWhere(
          'questions.version IN (SELECT MAX(Question_Draft.version) FROM Question_Draft WHERE Question_Draft.knowledgeAreaId = knowledgeArea.id AND questions.orderId = Question_Draft.orderId GROUP BY Question_Draft.orderId)',
        )
